...
test "document parser retains SGR color while removing control effects" {
    ...
    defer memory.deinit();
    const rendered = try parse(memory.allocator(), "\x1b[31mRed\x1b[0m safe\x1b[2J\x00\x1b]8;;https://invalid.example\x1b\\\nNext");
    try expectEqual(@as(usize, 2), rendered.lines.len);
    try expectEqualStrings("Red", rendered.lines[0].spans[0].text);
    try expectEqual(Color{ .indexed = 1 }, rendered.lines[0].spans[0].style.fg.?);
...
test "document parser handles RGB color and reset across lines" {
    var memory = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer memory.deinit();
    const rendered = try parse(memory.allocator(), "\x1b[1;38;2;12;34;56mBlue\nStill\x1b[0m plain");
    try expectEqual(Color{ .rgb = .{ 12, 34, 56 } }, rendered.lines[0].spans[0].style.fg.?);
    try expect(rendered.lines[0].spans[0].style.bold);
    try expectEqual(Color{ .rgb = .{ 12, 34, 56 } }, rendered.lines[1].spans[0].style.fg.?);
...
test "invalid document UTF-8 is rejected before it reaches terminal drawing" {
    var memory = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer memory.deinit();
    try std.testing.expectError(error.InvalidDocumentText, parse(memory.allocator(), "valid\xffinvalid"));
}
