...
fn repository(arena: std.mem.Allocator, io: std.Io) !Repo {
    var repo = try Repo.init(arena, io);
...
pub fn main(init: std.process.Init) !void {
    ...
    var ordinary = try repository(arena, io);
    ...
    var kinds = try repository(arena, io);
    defer kinds.deinit();
    ...
    var paths = try repository(arena, io);
    defer paths.deinit();
    ...
    var rename = try repository(arena, io);
    defer rename.deinit();
    ...
    var reverse = try repository(arena, io);
    ...
    var ambiguous = try repository(arena, io);
    ...
    var metadata = try repository(arena, io);
    ...
    var hunks = try repository(arena, io);
    defer hunks.deinit();
    ...
    try std.Io.Dir.cwd().writeFile(io, .{ .sub_path = "src/fixtures/colored.patch", .data = colored });
}
